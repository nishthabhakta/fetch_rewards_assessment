...
def users_transformation(file_path):
    ...
    #**********Converting the 13 digit unix timestamp to datetime format**********
    ...
    #**********Renaming the _id column to user_id and dropping duplicates to clean dataframe and ingest in database**********
...
def category_transformation(file_path):
    ...
    #**********Using only the category column from main dataframe (brands) to identify unique brand catgeories in order to normalize the database**********
    ...
    #**********Dropping out the null values and giving a unique id to each row of category**********
...
def reference_transformation(file_path):
    ...
    #**********Normalizing the cpg(nested json column) using json_normalize and storing it in dataframe called reference**********
...
def brands_transformation(file_path):
    ...
    #**********Normalizing the _id and cpg and renaming _id to brand_id column using json normalize**********
    ...
    #**********Dropping the category and cpg columns**********
    ...
    #**********Adding an empty column category_id to establish foreign key relationship in database and filling out the null values in columns to ingest clean data  in database*********
    ...
    #**********Concating the reference csv file to the cleaned brands dataframe to give reference ids to each row and dropping th reference_type columns**********
...
def receipts_transformation(file_path):
    ...
    #**********Reading the json files using pd.read_json and filling out the null values with 0**********
    ...
    #**********Normalizing column of _id using json_normalize**********
    ...
    #**********Normalizing the rewardsReceiptItemList column which is a nested json object in a dataframe**********
    ...
    #********** Dropping the rewardsReceiptItemListId column and renaming the columns in rewards_item dataframe and later dropping null values and filling empty rows with 0 using fillna **********
    ...
    #**********Manipulating and normalizing the main dataframe and dropping the normalized column 
    #rewardsReceiptItemList and renaming the _id column to receipt_id**********
    ...
    #********** Using regex to remove characters from all the date columns using the following code**********
    ...
    #**********Concating rewards and receipts dataframe and adding uuid to add a unique identifier for each row**********
    ...
    #*********Breaking the frame to ingest into database***********
    ...
    #********* Filling out empty rows of date columns with an old date ***********
...
def rewards_transformation(file_path):
    ...
    #********** Dropping the rewardsReceiptItemListId column and renaming the 
    #columns in rewards_item dataframe and later dropping null values and filling 
    #empty rows with 0 using fillna **********
    print("********** Dropping the rewardsReceiptItemListId column and renaming the columns in rewards_item dataframe and later dropping null values and filling empty rows with 0 using fillna **********")
    ...
    #**********Manipulating and normalizing the main dataframe and dropping the 
    #normalized column rewardsReceiptItemList and renaming the _id column to receipt_id**********
    print("**********Manipulating and normalizing the main dataframe and dropping the normalized column rewardsReceiptItemList and renaming the _id column to receipt_id**********")
...
#Code to ingest the transformed data into database
...
elif input_path.split("//")[-1].split(".")[0] == 'receipts':
...
elif input_path.split("//")[-1].split(".")[0] == 'users':
...
elif input_path.split("//")[-1].split(".")[0] == 'brands':
